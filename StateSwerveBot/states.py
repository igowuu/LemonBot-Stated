...
class FiniteStateMachine:
  ...
  def __init__(self, isInTest: bool):
    ...
    self._is_in_state = False

  def construct_state(self, name: str, conditional: Callable[[], bool],
                      runner: Callable[[], None], test_runner: Callable[[],
                                                                        None]):
  ...
  def execute(self):
    ...
    for state in self.states:
      if self._is_in_state:
        raise Exception("[ERROR] Already in a state")
      if state.conditional():
        self._is_in_state = True
